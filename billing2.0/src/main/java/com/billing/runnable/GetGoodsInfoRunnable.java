...
public class GetGoodsInfoRunnable implements Runnable {
    ...
    public GetGoodsInfoRunnable(String httpUrl, Handler mHandler, String req
    ) {
    ...
    @Override
    public void run() {
        String result = OkHttpClientRequest.get().SentRequest(httpUrl + "gwGetProductInfo.do", getCommit(req));
        try {
